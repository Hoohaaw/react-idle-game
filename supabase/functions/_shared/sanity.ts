// Server-side Sanity read used to validate authored content before a write. Mirrors the browser
// client (src/services/sanity.ts): same project/dataset and the `drafts` perspective, so unpublished
// character definitions still validate during the drafts-only authoring phase. The read token is a
...
export async function characterDefExists(charKey: string): Promise<boolean> {
  ...
  url.searchParams.set('query', 'count(*[_type == "characterDef" && charKey == $id])')
  url.searchParams.set('$id', JSON.stringify(charKey))
