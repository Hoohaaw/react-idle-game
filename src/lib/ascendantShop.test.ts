...
describe('resolveCharAscendantBonuses', () => {
  ...
  it('bundles only the 7 offense stats when power alone is invested', () => {
    const out = resolveCharAscendantBonuses({ 'lyra-swift.power': 2 }, 'lyra-swift')
    expect(out.attack).toEqual({ flat: 0, pct: 20 })
    expect(out.strength).toEqual({ flat: 0, pct: 20 })
    expect(out.agility).toEqual({ flat: 0, pct: 20 })
    expect(out.speed).toEqual({ flat: 0, pct: 20 })
    expect(out.intelligence).toEqual({ flat: 0, pct: 20 })
  ...
  it('bundles only health/defense when vitality alone is invested', () => {
    const out = resolveCharAscendantBonuses({ 'lyra-swift.vitality': 1 }, 'lyra-swift')
    expect(out.health).toEqual({ flat: 0, pct: 10 })
    expect(out.defense).toEqual({ flat: 0, pct: 10 })
